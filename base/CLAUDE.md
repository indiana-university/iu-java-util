...
Everything here is on the hot path of every other module and is held to full branch coverage, so a new utility method needs its failure branches exercised in the same commit. Prefer extending an existing class in the right family over adding a new top-level type.
