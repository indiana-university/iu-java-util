...
//	@Override
	/*
	 * protected ValidationResult validateAddress(SubjectConfirmation confirmation,
	 * Assertion assertion, ValidationContext context) throws
	 * AssertionValidationException { String address =
	 * StringSupport.trimOrNull(confirmation.getSubjectConfirmationData().getAddress
	 * ()); if (address == null) return ValidationResult.VALID;
	 * 
	 ...
	 * .format("Subject confirmation address '%s' is not resolvable hostname or IP address"
	 ...
	 * null) for (String range : allowedRanges) if
	 * (WebUtil.isInetAddressInRange(confirmingAddress, range)) {
	 * LOG.fine("Allowing whitelisted IP " + confirmingAddress + "; range = " +
	 * range); return ValidationResult.VALID; } }
	 * 
	 * ValidationResult result = super.validateAddress(confirmation, assertion,
	 * context); if (!ValidationResult.VALID.equals(result)) {
	 ...
	 * return ValidationResult.VALID; } return result; }
	 */
