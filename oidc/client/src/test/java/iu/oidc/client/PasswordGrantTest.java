...
public class PasswordGrantTest {
	...
	public void testGrantType() {
		...
		final var grant = new PasswordGrant(config);
		...
		assertEquals("password", params.get("grant_type").iterator().next());
		assertEquals(clientId, params.get("client_id").iterator().next());
		assertEquals(username, params.get("username").iterator().next());
		assertEquals(password, params.get("password").iterator().next());
